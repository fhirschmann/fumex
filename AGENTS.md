...
- Charge module upright in the plenum on the head floor (user, 2026-09-22: nothing may stick out, put the board in the airflow, and put it on the head floor). `plenum` grew to 14.3 and with it `body_d` to 74. Six wires now cross the head/base joint through the widened floor notch.
  - It sits in its own part `chg_holder`, screwed to the floor with 2 × M3 × 8. **That is not cosmetic:** anything standing in the plenum blocks the fan on its way out, and a bracket cast into the head would trap the fan for good. Removable holder, `fan_out` passes.
  - Two grooves take the short edges of the board, parts and all — the long edges are no good, the board carries parts up to them. Brackets 3.6 mm thick: at 3 the 2 mm groove left 1 mm of wall, which `analyze.py thickness` catches.
  - The brackets start at `chg_y`, not in front of it: the heatsink overhangs the board's IN end by 4.5 mm and clipped the right one. The bracket ramp starts below the heatsink for the same reason.
  - The back cover's lip is notched over the whole holder, otherwise its bottom band sits on the holder plate.
