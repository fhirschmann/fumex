...
The filter cassette is held by four embedded magnet pairs and comes off by hand; two finger scoops in the intake face give you something to pull against. Each magnet sits in a closed Ø10.3 × 3.2 mm cavity with 1.2 mm of solid plastic on both sides. Head and cassette prints pause before sealing the cavities. The cassette stands 5.6 mm proud of the intake face, with a continuous 1.2 mm bevel and 4.4 mm of straight rim.

The charge module stands upright on the ballast lid at the back of the bay. A holder and one cable tie secure only its cool OUT end; the heatsink end stays free, with 4.7 mm between the printed holder and the heatsink. Its components face forwards and its heatsink faces the back wall, exposing both sides of the hot end to air. Five slots and the larger cable notch connect to the plenum behind the fan, with passages past the component side and the heatsink to the back-wall ventilation slots. This branch carries filtered air because it starts downstream of the mat. With the fan off, the same openings allow natural convection. Actual cooling must be checked with the assembled wiring and the housing closed.
...
| `head_back` back cover | 1 | PETG black | 145 × 144.7 × 15.9 | outside on the bed |
...
| `cassette` filter cassette | 1 | PETG grey | 138 × 143 × 5.6 | grid face on the bed |
...
| Neodymium disc magnets Ø 10 × 3 | 8 | – | four pairs, inserted during the head/cassette print pauses and enclosed by plastic |
...
For an earlier printed version, this revision changes the head, cassette and back cover. Use the matching back cover with its shorter fan posts; the base and support cross can be reused.
...
2. Insert the magnets during printing: the head and cassette plates each pause before the **4.6 mm layer** with all four cavities open. Mark the same magnetic pole on all eight discs beforehand, then place four discs **marked face up on both plates**; this gives attracting faces after assembly. Seat each disc flat, below the surrounding printed surface, before resuming. The next layers enclose them; no glue is needed. Use the project 3MF for the saved pauses; an STL alone does not carry pause instructions.
